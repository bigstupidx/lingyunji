...
public class AnimateTiledTexture : MonoBehaviour
{
    ...
    public void RegisterCallback(VoidEvent cbFunction)
    {
        ...
        if (_enableEvents)
            ...
            Debuger.LogWarning("AnimateTiledTexture: You are attempting to register a callback but the events of this object are not enabled!");
    }
    ...
    public void UnRegisterCallback(VoidEvent cbFunction)
    {
        ...
        if (_enableEvents)
            ...
            Debuger.LogWarning("AnimateTiledTexture: You are attempting to un-register a callback but the events of this object are not enabled!");
    }
    ...
    public void ChangeMaterial(Material newMaterial, bool newInstance = false)
    {
        if (newInstance)
        {
            ...
            if (_hasMaterialInstance)
                Object.Destroy(mRenderer.sharedMaterial);
 
            // create the new material
            _materialInstance = new Material(newMaterial);

            // Assign it to the renderer
            mRenderer.sharedMaterial = _materialInstance;
        ...
        mRenderer.sharedMaterial.SetTextureScale("_MainTex", _textureSize);
    }
...
//#endif
        ...
        ChangeMaterial(mRenderer.sharedMaterial, _newMaterialInstance);
    }
    ...
    private IEnumerator updateTiling()
    {
        ...
        WaitForSeconds m_waitTime = new WaitForSeconds(1f / _framesPerSecond);
        while (true)
        {
