...
{
	[CustomEditor (typeof(DepthOfFieldDeprecatedBase))]
	public class DepthOfFieldDeprecatedBaseEditor : Editor
    {
        ...
        public override void OnInspectorGUI () {
			...
			GameObject go = (target as DepthOfFieldDeprecatedBase).gameObject;

            if (!go)
                return;

            Camera camera = (target as DepthOfFieldDeprecatedBase).GetCamera();
            if (camera == null)
                return;

            if (simpleTweakMode.boolValue)
                GUILayout.Label ("Current: "+ camera.name+", near "+ camera.nearClipPlane+", far: "+camera.farClipPlane+", focal: "+focalPoint.floatValue, EditorStyles.miniBoldLabel);
            else
                GUILayout.Label ("Current: "+ camera.name+", near "+ camera.nearClipPlane+", far: "+ camera.farClipPlane+", focal: "+focalZDistance.floatValue, EditorStyles.miniBoldLabel);
            ...
            if (simpleTweakMode.boolValue) {
                focalPoint.floatValue = EditorGUILayout.Slider ("Focal distance", focalPoint.floatValue, camera.nearClipPlane, camera.farClipPlane);
                ...
                focalSize.floatValue = EditorGUILayout.Slider ("Focal size", focalSize.floatValue, 0.0f, (camera.farClipPlane - camera.nearClipPlane));
            }
            else {
                focalZDistance.floatValue = EditorGUILayout.Slider ("Distance", focalZDistance.floatValue, camera.nearClipPlane, camera.farClipPlane);
                EditorGUILayout.PropertyField (objectFocus, new GUIContent("Transform"));
                focalSize.floatValue = EditorGUILayout.Slider ("Size", focalSize.floatValue, 0.0f, (camera.farClipPlane - camera.nearClipPlane));
