...
namespace Eyesblack.EditorTools {
    public class RenderCubemapWizard : EditorWindow {
        public enum GenMode {
            SingleCubemap = 0,
            SixTextures
        }
        ...
        private Dictionary<string, bool> _componentStates;
        private List<string> _listModified = new List<string>();
        ...
        IEnumerable<Behaviour> GetValidCameraComponents(Camera cam) {
            foreach (Behaviour comp in cam.GetComponents<Behaviour>()) {
                if (comp != null) {
                    System.Type type = comp.GetType();
                    if (type != typeof(Transform) && type != typeof(Camera)) {
                        yield return comp;
                    }
                }
            }
        ...
        private Texture2D WriteAndImportTexture() {
            if (_genMode == GenMode.SingleCubemap) {
                ...
                if (!string.IsNullOrEmpty(path)) {
                    ...
                    //material.SetTexture("_FrontTex", newTextures[4]);
                    //material.SetTexture("_BackTex", newTextures[5]);
                    //material.SetTexture("_LeftTex", newTextures[0]);
                    //material.SetTexture("_RightTex", newTextures[1]);
                    //material.SetTexture("_UpTex", newTextures[2]);
                    //material.SetTexture("_DownTex", newTextures[3]);
        ...
        [MenuItem("Eyesblack/渲染到环境贴图")]
        private static void Init() {
            RenderCubemapWizard editWindow = ScriptableObject.CreateInstance<RenderCubemapWizard>();
            editWindow.Show();
        }
    }
}
