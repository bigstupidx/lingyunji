...
//	////////////////////////////////
	...
	void OnDrawGizmosSelected()
    {
    	...
    	Handles.color = myCol;
    	if(SyncWindDir) {
    		Handles.ArrowHandleCap(0, transform.position, transform.rotation, hsize, EventType.Repaint);
    	}
    	else {
    		Quaternion rotation = Quaternion.LookRotation(new Vector3(Wind.x, Wind.y, Wind.z));
    		Handles.ArrowHandleCap(0, transform.position, rotation, hsize, EventType.Repaint);
    	}
    }
    #endif
