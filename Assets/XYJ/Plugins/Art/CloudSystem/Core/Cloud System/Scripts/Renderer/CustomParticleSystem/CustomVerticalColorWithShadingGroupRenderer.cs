...
namespace Edelweiss.CloudSystem {
	...
	/// <summary>
	/// Generic base class of <see cref="T:CS_CustomVerticalColorWithShadingGroupRenderer"/>.
	...
	/// </typeparam>
	public abstract class CustomVerticalColorWithShadingGroupRenderer <C, PD, CD> : CustomCloudRenderer <C, PD, CD>
		...
		protected override void UpdateMesh (object a_State) {
			...
			for (int i = l_LeftIndex; i < l_RightIndex; i = i + 1) {
				...
				m_UVs [l_Index + 3] = m_UVLookupTable [l_Particle.uvIndex, 3];
					...
					// Vertical color
				float l_HeightFactor1 = m_CloudRendererData.HeightToVerticalFactor (l_Position1.y);
				float l_HeightFactor2 = m_CloudRendererData.HeightToVerticalFactor (l_Position2.y);
				float l_HeightFactor3 = m_CloudRendererData.HeightToVerticalFactor (l_Position3.y);
				float l_HeightFactor4 = m_CloudRendererData.HeightToVerticalFactor (l_Position4.y);
				...
				Color l_VertexColor1 = (m_CloudRendererData.VerticalShadingInfluence * m_CloudRendererData.VerticalColorAt (l_HeightFactor1)) + (m_CloudRendererData.ShadingGroupInfluence * l_SunInfluence1);
				Color l_VertexColor2 = (m_CloudRendererData.VerticalShadingInfluence * m_CloudRendererData.VerticalColorAt (l_HeightFactor2)) + (m_CloudRendererData.ShadingGroupInfluence * l_SunInfluence2);
				Color l_VertexColor3 = (m_CloudRendererData.VerticalShadingInfluence * m_CloudRendererData.VerticalColorAt (l_HeightFactor3)) + (m_CloudRendererData.ShadingGroupInfluence * l_SunInfluence3);
				Color l_VertexColor4 = (m_CloudRendererData.VerticalShadingInfluence * m_CloudRendererData.VerticalColorAt (l_HeightFactor4)) + (m_CloudRendererData.ShadingGroupInfluence * l_SunInfluence4);
