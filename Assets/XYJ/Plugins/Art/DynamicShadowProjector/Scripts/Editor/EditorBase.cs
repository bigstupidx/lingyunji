...
namespace DynamicShadowProjector.Editor {
	public class EditorBase : UnityEditor.Editor {
		protected static GUIContent[] s_textureSizeDisplayOption = new GUIContent[] {new GUIContent("16"), new GUIContent("32"), new GUIContent("64"), new GUIContent("128"), new GUIContent("256"), new GUIContent("512"), new GUIContent("1024")};
		protected static int[] s_textureSizeOption = new int[] {16, 32, 64, 128, 256, 512,1024};
		...
		private static bool RemoveUnuserMaterialPropertyData(Material mat, SerializedProperty prop, string forceRemoveProperty)
		{
			...
			for (int i = 0; i < prop.arraySize; ++i) {
                ...
                SerializedProperty first = elem.FindPropertyRelative("first");
                if (first == null)
                    continue;
                SerializedProperty namePro = first.FindPropertyRelative("name");
                if (namePro == null)
                    continue;
