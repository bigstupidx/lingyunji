...
namespace PigeonCoopToolkit.Effects.Trails
{
    public abstract class TrailRenderer_Base : MonoBehaviour
    {
		...
		public bool FollowRootPosition;
		public bool FollowRootRotation;
        ...
        public void CheckRenderQueue()
        {
            //自定义渲染层级
            if (TrailData.TrailMaterial != null && TrailData.UseCustomRenderQueue && TrailData.renderQueue != TrailData.TrailMaterial.renderQueue)
            {
                TrailData.TrailMaterial.renderQueue = TrailData.renderQueue;
            }
        ...
        {
            ///Fix 隐藏再显示的时候会有瞬间的连线的问题，这里把点的激活状态改下就没问题了，不需要删除_activeTrail或者删除_activeTrail的mesh
            if (_activeTrail != null)
            {
                for (int i = 0; i < _activeTrail.Points.Count; i++)
                {
                    _activeTrail.Points[i].SetTimeActive(TrailData.Lifetime + 1);
                }
            }
            if (_fadingTrails != null)
            {
                foreach (PCTrail fadingTrail in _fadingTrails)
                {
                    ...
                    {
                        fadingTrail.Points[i].SetTimeActive(TrailData.Lifetime + 1);
                    }
                }
            }
        ...
        protected void AddPoint(PCTrailPoint newPoint, Vector3 pos)
        {
			...
			if (Root != null) {
				if (FollowRootPosition && FollowRootRotation)
					pos = Root.InverseTransformPoint(pos);
				else if (FollowRootPosition && !FollowRootRotation)
					pos -= Root.position;
				else if (!FollowRootPosition && FollowRootRotation) {
					pos = Root.rotation * pos;
				}
			}
        ...
        private void DrawMesh(Mesh trailMesh, Material trailMaterial)
        {
			...
			if (Root != null) {
				if (FollowRootPosition && FollowRootRotation)
					matrix = Root.localToWorldMatrix;
				else if (FollowRootPosition && !FollowRootRotation) {
					matrix.m03 += Root.position.x;
					matrix.m13 += Root.position.y;
					matrix.m23 += Root.position.z;
				}
				else if (!FollowRootPosition && FollowRootRotation) {
					matrix.SetTRS(Vector3.zero, Root.rotation, Vector3.one);
				}
			}

			Graphics.DrawMesh(trailMesh, matrix, trailMaterial, gameObject.layer);
        }
