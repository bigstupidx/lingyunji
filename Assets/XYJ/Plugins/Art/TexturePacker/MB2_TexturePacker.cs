...
//						img.y = r.y;
        ...
        public Rect[] GetRects<T>(List<T> imgWidthHeights, int maxDimension, int padding, out int outW, out int outH, System.Func<T, Vector2> getSize)
        {
            List<Vector2> sizes = new List<Vector2>(imgWidthHeights.Count);
            int total = 0;
            for (int i = 0; i < imgWidthHeights.Count; ++i)
            {
                Vector2 s = getSize(imgWidthHeights[i]);
                sizes.Add(s);
                total += (int)(s.x * s.y);
            }

            if (total >= maxDimension * maxDimension)
            {
                outW = 0;
                outH = 0;
                return null;
            ...
            return _GetRects(sizes, maxDimension, padding, 2 + padding * 2, 2 + padding * 2, 2 + padding * 2, 2 + padding * 2, out outW, out outH, 0);
        }
        ...
        Rect[] _GetRects(List<Vector2> imgWidthHeights, int maxDimension, int padding, int minImageSizeX, int minImageSizeY, int masterImageSizeX, int masterImageSizeY, out int outW, out int outH, int recursionDepth){
			...
			while (numWIterations >= 1 && steppedHeight < sqrtArea * 1000){	
				...
				while (!successW && steppedWidth < sqrtArea * 1000){	
					...
					if (Probe(imgsToAdd, steppedWidth, steppedHeight, area, maxDimension, pr)){
                        ...
                        Debug.Log("increasing Width h=" + steppedHeight + " w=" + steppedWidth);
					}			
				}
				steppedHeight = StepWidthHeight(steppedHeight,stepH,maxDimension);
                Debug.Log("increasing Height h=" + steppedHeight + " w=" + steppedWidth);
			}
            ...
            Debug.Log("Best fit found: atlasW=" + outW + " atlasH" + outH + " w=" + bestRoot.w + " h=" + bestRoot.h + " efficiency=" + bestRoot.efficiency + " squareness=" + bestRoot.squareness + " fits in max dimension=" + bestRoot.fitsInMaxSize);
			...
			if (!redoPacking){
				...
				for (int i = 0; i < images.Count; i++){
                    ...
                    Rect r = rs[i] = new Rect(im.x + padding, im.y + padding, im.w - padding * 2, im.h - padding * 2);
                    Debug.Log("Image: " + i + " imgID=" + im.imgId + " x=" + r.x +
                               " y=" + r.y + " w=" + r.width +
                               " h=" + r.height + " padding=" + padding);
                }
			} else {
