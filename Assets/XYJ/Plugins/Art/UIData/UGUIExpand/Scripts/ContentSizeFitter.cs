...
namespace xys.UI
{
    ...
    public class ContentSizeFitter : UIBehaviour, ILayoutSelfController
    {
        ...
        [SerializeField]
        public Vector2[] m_RangeSize = new Vector2[2]; // 如果为限定范围，数值
        ...
        private void HandleSelfFittingAlongAxis(int axis)
        {
            ...
            else if (fitting == FitMode.RangeSize)
            {
                float lenght = LayoutUtility.GetPreferredSize(m_Rect, axis);
                Vector2 size = m_RangeSize[axis];
                lenght = Mathf.Clamp(lenght, size.x, size.y);

                rectTransform.SetSizeWithCurrentAnchors((RectTransform.Axis)axis, lenght);
            }
            else
            {
