...
namespace UI
{
    public class UITexturePacker
    {
        ...
        public static Rect[] PackTextures<T>(out Vector2 texture, IList<T> textures, System.Func<T, Vector2> fun, int width, int height, int padding, int maxSize)
        {
            int total = 0;
            Vector2[] sizes = new Vector2[textures.Count];
            for (int i = 0; i < textures.Count; ++i)
            {
                sizes[i] = fun(textures[i]);
                total += (int)((sizes[i].x + 1) * (sizes[i].y + 1));
            }

            if (total * 0.9f >= maxSize * maxSize)
            {
                texture = Vector2.zero;
                return null;
            }

            return PackTextures(out texture, sizes, width, height, padding, maxSize);
        }

        public static Rect[] PackTextures(out Vector2 texture, Vector2[] textures, int width, int height, int padding, int maxSize)
        {
            int totalsize = 0;
            List<XTools.Pair<Vector2, int>> sorts = new List<XTools.Pair<Vector2, int>>();
            for (int i = 0; i < textures.Length; ++i)
                sorts.Add(new XTools.Pair<Vector2, int>(textures[i], i));

            sorts.Sort((XTools.Pair<Vector2, int> x, XTools.Pair<Vector2, int> y) =>
            {
                return (x.first.x * x.first.y).CompareTo(y.first.x * y.first.y);
            });

            Vector2[] ss = new Vector2[sorts.Count];
            for (int i = 0; i < sorts.Count; ++i)
            {
                ss[i] = sorts[i].first;
                totalsize += (int)(ss[i].x * ss[i].y);
            }
            ...
            foreach (int max in maxs)
            {
                for (int i = 0; i < 5; ++i)
                {
                    Rect[] rects = PackTextures(out texture, ss, width, height, padding, max, (FreeRectChoiceHeuristic)i);
                    if (rects != null)
                    {
                        Rect[] crs = new Rect[rects.Length];
                        for (int m = 0; m < sorts.Count; ++m)
                        {
                            crs[sorts[m].second] = rects[m];
                        }
        ...
        static Rect[] PackTextures(out Vector2 texture, Vector2[] textures, int width, int height, int padding, int maxSize, FreeRectChoiceHeuristic method)
        {
            texture = Vector2.zero;
            ...
            for (int i = 0; i < textures.Length; i++)
            {
                ...
                for (xPadding = 1; xPadding >= 0; --xPadding)
                {
                    ...
                    {
                        rect = bp.Insert((int)textures[i].x + (xPadding * padding), (int)textures[i].y + (yPadding * padding), method);
                        ...
                        {
                            return PackTextures(out texture, textures, width * (width <= height ? 2 : 1), height * (height < width ? 2 : 1), padding, maxSize, method);
                        }
                    }
