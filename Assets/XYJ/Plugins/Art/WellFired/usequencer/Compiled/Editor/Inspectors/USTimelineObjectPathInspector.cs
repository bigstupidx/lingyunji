...
namespace WellFired
{
	...
	public class USTimelineObjectPathInspector : Editor 
	{
        ...
        Vector3 EditorOffsetPosition = Vector3.zero;
		...
		public override void OnInspectorGUI()
		{
			...
			if(objectTimelinePaths.Count == 1)
			{
				...
				if(keyControls)
				{
                    EditorOffsetPosition = EditorGUILayout.Vector3Field("统一偏移量", EditorOffsetPosition);
                    if (GUILayout.Button("生效"))
                    {
                        foreach (var keyframe in objectTimelinePaths[0].Keyframes)
                            keyframe.Position += EditorOffsetPosition;
                    ...
                    foreach (var keyframe in objectTimelinePaths[0].Keyframes)
					{
						using(new WellFired.Shared.GUIBeginHorizontal())
						{
                            ...
                            EditorGUILayout.Vector3Field("", keyframe.Position);
			...
			if(serializedObject.ApplyModifiedProperties() || isUpdate)
			{
