...
namespace WellFired
{
	...
	public abstract class IUSHierarchyItem : ScriptableObject
	{
		...
		/// <summary>
		/// If you return true from this property, your item will show a More button on the floating pane, when the user clicks this, 
		/// AddContextItems will be called on your timeline, from there you can perform custom logic.
		/// </summary>
		/// <value><c>true</c> if should display more button; otherwise, <c>false</c>.</value>
		public virtual bool ShouldDisplayMoreButton { get; set; }
		...
		/// <summary>
		/// If your item has selectable objects, you should return true from this method. By default, this method returns false if the hierarchy item is not expanded.
		...
		protected virtual void FloatingOnGUI(int depth, Rect parentRect)
		{
			...
			{
				float size = 16.0f;
				var moreButtonRect = new Rect(parentRect.width - (size * 1.0f), parentRect.y, size, size);
