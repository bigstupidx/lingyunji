...
namespace WellFired
{
	...
	public class USPropertyTimelineHierarchyItem : IUSTimelineHierarchyItem 
	{
		...
		private PropertyBox removingProperty = null;
			...
			using(new WellFired.Shared.GUIBeginHorizontal())
			{
				using(new WellFired.Shared.GUIBeginVertical(GUILayout.MaxWidth(FloatingWidth)))
				{
					...
					if(IsExpanded)
					{
						...
						using(new WellFired.Shared.GUIBeginArea(propertyArea))
						{
							foreach(var propertyBox in propertyBoxes)
							{
								using(new WellFired.Shared.GUIBeginHorizontal())
								{
									...
									string preFix = propertyBox.PropertyFieldInfo.Name;
									USPropertyInfo propertyInfo = PropertyTimeline.GetProperty(preFix, propertyBox.PropertyFieldInfo.Component);
									using(new WellFired.Shared.GUIChangeColor(propertyInfo.UseCurrentValue ? Color.red : GUI.color))
									{
		...
		public override void Initialize(USTimelineBase timeline)
		{
			...
			if(PropertyTimeline.AffectedObject == null || CurveEditor == null || PropertyTimeline.Properties == null)
				return;

            PropertyTimeline.Properties.RemoveAll((USPropertyInfo info) => { return info == null ? true : false; });
		...
		private void RemoveProperty(PropertyBox propertyBox)
		{
			...
			string preFix = propertyBox.PropertyName;
			USPropertyInfo propertyInfo = PropertyTimeline.GetProperty(preFix, propertyBox.Component);
