...
namespace WellFired
{
    ...
    public class PropertyBoxPopup : EditorWindow
	{
		...
		private static float stored = 0.0f;
		...
		{
			Event.current.Use();
			if(PropertyBoxPopup.addPropertyBoxPopup == null)
				PropertyBoxPopup.addPropertyBoxPopup = ScriptableObject.CreateInstance<PropertyBoxPopup>();
 		
			PropertyBoxPopup.addPropertyBoxPopup.Init(buttonRect, propertiesToShow, CommitModificationsDelegate);
		}
		...
		private void Init(Rect buttonRect, List<PropertyBox> propertiesToShow, Action<IEnumerable<PropertyBox>> CommitModificationsDelegate)
		{
			...
			Array array = Array.CreateInstance(popupLocationHelperType, 1);
			array.SetValue(enumValue, 0);

            var methods = typeof(EditorWindow).GetMethods(BindingFlags.Instance | BindingFlags.NonPublic);

            MethodInfo method = null;
            for (int i = 0; i < methods.Length; ++i)
            {
                if (methods[i].Name == "ShowAsDropDown")
                {
                    var Parameters = methods[i].GetParameters();
                    Type[] types = new Type[] { typeof(Rect), typeof(Vector2), popupLocationHelperType };
                    if (types.Length == Parameters.Length)
                    {
                        for (int m = 0; m < Parameters.Length; ++m)
                        {
                            if (Parameters[m].ParameterType != types[m])
                                continue;
                        }
            ...
            try
            {
				method.Invoke(this, new object[] { buttonRect, PropertyBoxPopup.windowSize, array });
			}
