...
namespace WellFired
{
	public static class PropertyFieldInfoUtility
	{
		...
		private static Dictionary<Type, List<string>> SuggestionList = new Dictionary<Type, List<string>>()
		{
			{ typeof(Transform), 		new List<string>() { "localPosition", "localRotation", } },
			{ typeof(Camera), 			new List<string>() { "aspect", "fieldOfView", } },
		};

		public static bool IsSuggestedProperty(Component component, string propertyName)
		{
			var suggestedProperties = new List<string>();
			var found = SuggestionList.TryGetValue(component.GetType(), out suggestedProperties);

			if(!found)
				return false;

			return suggestedProperties.Contains(propertyName);
		}
	}
}