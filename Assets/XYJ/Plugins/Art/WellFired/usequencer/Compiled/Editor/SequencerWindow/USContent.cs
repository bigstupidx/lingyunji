...
namespace WellFired
{
	...
	public class USContent : ScriptableObject 
	{
		...
		private Texture DragAreaTexture
		{
			get
			{
                if (!cachedDragTexture)
                    cachedDragTexture = USEditorUtility.EditorResourcesLoad<Texture>("DragAreaTexture.png");
		...
		private void OnItemClicked(IUSHierarchyItem item)
		{
			...
			if(timelineContainerItem != null && timelineContainerItem.TimelineContainer != null && timelineContainerItem.TimelineContainer.AffectedObject != null && !USRuntimeUtility.HasObserverTimeline(timelineContainerItem.TimelineContainer.transform))
			{
