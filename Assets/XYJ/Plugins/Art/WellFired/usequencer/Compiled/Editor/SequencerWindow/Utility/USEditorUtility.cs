...
namespace WellFired
{
	...
	public static class USEditorUtility 
	{
        ...
        public static UnityEngine.Object EditorResourcesLoad(string assetPath)
        {
            return EditorResourcesLoad<UnityEngine.Object>(assetPath);
        }

        public static T EditorResourcesLoad<T>(string assetPath) where T : UnityEngine.Object
        {
            bool hassuffix = true;
            ...
            T v = AssetDatabase.LoadAssetAtPath<T>("Assets/Art/WellFired/usequencer/Uncompiled/Editor/Res/" + assetPath);

            if (v == null && !hassuffix)
            {
                assetPath = assetPath.Substring(0, assetPath.LastIndexOf('.')) + "jpg";
                return AssetDatabase.LoadAssetAtPath<T>("Assets/Art/WellFired/usequencer/Uncompiled/Editor/Res/" + assetPath);
            }
		...
		static public GUISkin USeqSkin
		{
			get
			{
				if(!uSeqSkin)
				{
                    ...
                    uSeqSkin = EditorResourcesLoad<GUISkin>(Skin + ".guiskin");
				}
		...
		public static Texture TimelineMarker
		{
			get
			{
				if(timelineMarker == null)
					timelineMarker = EditorResourcesLoad<Texture>("TimelineMarker");
		...
		public static Texture TimelineScrubHead
		{
			get
			{
				if(timelineScrubHead == null)
					timelineScrubHead = USEditorUtility.EditorResourcesLoad("TimelineScrubHead") as Texture;
		...
		static public Texture LoadTexture(string textureName)
		{
			...
			string fullFilename = String.Format("{0}/{1}.png", directoryName, textureName);
			return EditorResourcesLoad<Texture>(fullFilename); 
		}
		...
		public static List<string> CollectAllUnusedAssetsFromSequence(USSequencer sequence)
		{
			...
			var relativePath = System.IO.Path.GetDirectoryName(prefabName) + "/" + prefabParent.name;
		...
		public static void CreatePrefabFrom(USSequencer sequence, bool quiet)
		{
			...
			else
			{
				...
				relativePath = System.IO.Path.GetDirectoryName(prefabName) + "/" + parentPrefab.name;
