	...
	public class USPropertyInfo : ScriptableObject
	{
		...
		public FieldInfo fieldInfo
		{
			get 
			{
				...
				if(!component || string.IsNullOrEmpty(fieldName)) 
					return null;
		...
		public bool UseCurrentValue 
		{
			get
			{
				return curves.Any(curve => curve.UseCurrentValue);
			}
			set
			{
				foreach(var curve in curves)
					curve.UseCurrentValue = value;
			}
		}
		...
		{
			var componentName = string.Format("\"ComponentType\": \"{0}\"", ComponentType);
			var propertyName = string.Format("\"ComponentName\": \"{0}\"", PropertyName);
			return string.Format("{{{0}, {1}}}", componentName, propertyName);
		}
	}
}