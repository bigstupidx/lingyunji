...
namespace WellFired
{
	...
	public class USTimelineObserver : USTimelineBase 
	{
		...
		public override void Process(float sequencerTime, float playbackRate)
		{
			...
			for(int index = 0; index < observerKeyframes.Count; index++)
			{
		...
		public bool IsValidCamera(Camera testCamera)
		{
			...
			var isPrefab = WellFired.Shared.PlatformSpecificFactory.UnityEditorHelper.IsPrefab(testCamera.gameObject);	

			if(notEdittable || hideAndDontSave || hideInHierarchy || isPrefab)
				return false;
