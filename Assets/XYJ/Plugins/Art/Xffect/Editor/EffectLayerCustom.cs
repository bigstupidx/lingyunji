...
public class EffectLayerCustom : Editor
{
    ...
    protected SerializedProperty SpriteXYDir;
    ...
    protected SerializedProperty GravityInheritRotation;
    ...
    protected SerializedProperty UseFlowControl;

    protected SerializedProperty ShaderCurveX1;
    protected SerializedProperty ShaderCurveX2;
    protected SerializedProperty FlowSpeed;
    protected SerializedProperty FlowStretchLength;


    protected SerializedProperty DecalSize;


    protected SerializedProperty STBezierSpline;
    //protected SerializedProperty STGranularity;
    protected SerializedProperty STElementWidth;
    protected SerializedProperty STTrailLen;
    protected SerializedProperty RotationAxis;
    protected SerializedProperty EmitSpline;
    protected SerializedProperty EmitSplinePosType;
    protected SerializedProperty SplineEaseType;

    protected SerializedProperty TimeScaleEnabled;
    protected SerializedProperty TimeScaleCurve;
    ...
    void InitSerializedProperty()
    {
        ...
        SpriteXYDir = serializedObject.FindProperty("SpriteXYDir");
        ...
        GravityInheritRotation = serializedObject.FindProperty("GravityInheritRotation");
        ...
        UseFlowControl = serializedObject.FindProperty("UseFlowControl");
        ...
        FlowStretchLength = serializedObject.FindProperty("FlowStretchLength");

        DecalSize = serializedObject.FindProperty("DecalSize");


        //spline trail config
        STBezierSpline = serializedObject.FindProperty("STBezierSpline");
        //STGranularity = serializedObject.FindProperty("STGranularity");
        STElementWidth = serializedObject.FindProperty("STElementWidth");
        STTrailLen = serializedObject.FindProperty("STTrailLen");
        RotationAxis = serializedObject.FindProperty("RotationAxis");

        EmitSpline = serializedObject.FindProperty("EmitSpline");

        EmitSplinePosType = serializedObject.FindProperty("EmitSplinePosType");
        SplineEaseType = serializedObject.FindProperty("SplineEaseType");

        TimeScaleEnabled = serializedObject.FindProperty("TimeScaleEnabled");
    ...
    void DrawTimeScale()
    {
        XEditorTool.XArea m = XEditor.BeginXArea("TimeScale Controller", EffectLayerCustom.XArea, XEditorTool.MinHeight, XEditorTool.EArea.CheckBox, TimeScaleEnabled);
        if (!m.Open)
        {
            XEditor.EndXArea();
            return;
        ...
        DrawCurveX(TimeScaleCurve, "TimeScale Curve");
    ...
    void DrawShaderController()
    {
        ...
        bool showFlowControl = shaderName.Contains("Xffect/displacement-flow");

        if (!showDisplacementControl && !showDissolveControl && !showFlowControl)
        {
            UseShaderCurve1.boolValue = false;
            UseShaderCurve2.boolValue = false;
            UseFlowControl.boolValue = false;
            return;
        }
        ...
        //note currently flow shader is not support to use displacement control, param count is not enough:(

        if (showFlowControl)
        {
            if (XEditor.DrawToggle("control flow?", "", UseFlowControl))
            {
                XEditor.DrawFloat("speed:", "", FlowSpeed);
                XEditor.DrawFloat("stretch length:", "", FlowStretchLength);
            }

            UseShaderCurve1.boolValue = UseShaderCurve2.boolValue = false;
    ...
    void DrawCollision()
    {
        ...
        if (CollisionAutoDestroy.boolValue && IsNodeLifeLoop.boolValue)
        {
            XEditor.DrawInfo("Node with infinite life can't be destroyed.");
        }
    ...
    void DrawRotationConfig()
    {
        ...
        if (RenderType.intValue == 6)
        {
            XEditor.DrawInfo("note the spline trail's rotation is in the local space of the spline obj");
            XEditor.DrawVector3Field("rotate axis:", "", RotationAxis);
        }
        ...
        else if ((RSTYPE)RotateType.enumValueIndex == RSTYPE.SIMPLE)
        {
            ...
            {
                XEditor.DrawInfo("Billboard self sprite is not recommended to rotate, it might not work properly.");
            }
        }
    ...
    void DrawSplineTrail()
    {
        XEditorTool.XArea m = XEditor.BeginXArea("SplineTrail Config", XArea, XEditorTool.MinHeight, XEditorTool.EArea.AlwaysToggle, null);
        if (!m.Open)
        {
            XEditor.EndXArea();
            return;
        ...
        XEditor.DrawInfo("note the trail's smoothness is affected by the spline obj's 'Granularity' parameter.");

        GUIContent content = new GUIContent("spline obj:");

        EditorGUILayout.BeginHorizontal();
        EditorGUILayout.LabelField(content, EffectLayerCustom.XLabelField, GUILayout.Width(XEditorTool.LabelWidth));
        STBezierSpline.objectReferenceValue = EditorGUILayout.ObjectField(STBezierSpline.objectReferenceValue, typeof(XSplineComponent), true);
        EditorGUILayout.EndHorizontal();

        //XEditor.DrawInt("granularity:", "", STGranularity);
        //XEditor.DrawFloat("trail len:", "", STTrailLen);
        XEditor.DrawFloat("trail width:", "", STElementWidth);
    ...
    void DrawCustomMesh()
    {
        ...
        XEditor.DrawVector3Field("rotate&scale axis:", "mesh rotation and scale will use this axis.", MeshRotateAxis);
    ...
    void DrawGravityAffector()
    {
        ...
        if ((GAFTTYPE)GravityAftType.enumValueIndex == GAFTTYPE.Planar)
        {
            ...
            XEditor.DrawToggle("inherit rotation", "", GravityInheritRotation);
        }
    ...
    void DrawDirectionConfig()
    {
        ...
        else if ((DIRECTION_TYPE)DirType.enumValueIndex == DIRECTION_TYPE.Spline)
        {
            if (EmitType.intValue != 6)
            {
                XEditor.DrawInfo("You need to set the emitter shape to 'Spline'");
                XEditor.EndXArea();
                return;
            }
            ...
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(content, EffectLayerCustom.XLabelField, GUILayout.Width(XEditorTool.LabelWidth));
            EmitSpline.objectReferenceValue = EditorGUILayout.ObjectField(EmitSpline.objectReferenceValue, typeof(XSplineComponent), true);
            EditorGUILayout.EndHorizontal();

            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField("ease type:", XLabelField, GUILayout.Width(XEditorTool.LabelWidth));
            SplineEaseType.enumValueIndex = (int)(XEase.Easing)EditorGUILayout.EnumPopup((XEase.Easing)SplineEaseType.enumValueIndex);
            EditorGUILayout.EndHorizontal();
    ...
    void DrawEmitterConfig()
    {
        ...
        if (RenderType.intValue == 6)
        {
            //XEditor.DrawInfo("note the spline trail's emit position is in the local space of the spline obj");
        }
        ...
        else if (EmitType.intValue == 6)
        {
            ...
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(new GUIContent("start pos:", ""), XLabelField, GUILayout.Width(XEditorTool.LabelWidth));
            EmitSplinePosType.intValue = (int)(EMITTYPE)EditorGUILayout.EnumPopup((EEmitSplinePos)EmitSplinePosType.intValue);
            EditorGUILayout.EndHorizontal();

            //IMPORTANT!
            SyncClient.boolValue = false;
        }
        ...
        int valueBefore = MaxENodes.intValue;
        XEditor.DrawInt("max nodes:", TMaxENodes, MaxENodes);
        needToReset = false;
        if (MaxENodes.intValue != valueBefore)
        {
            if (Script.gameObject.transform.parent != null)
            {
                XffectComponent parent = Script.gameObject.transform.parent.GetComponent<XffectComponent>();
    ...
    void DrawDecal()
    {
        XEditorTool.XArea m = XEditor.BeginXArea("Decal", XArea, XEditorTool.MinHeight, XEditorTool.EArea.AlwaysToggle, null);
        if (!m.Open)
        {
            XEditor.EndXArea();
            return;
        }
        XEditor.DrawVector3Field("size:", "", DecalSize);
    ...
    void DrawSpriteConfig()
    {
        ...
        if (SpriteType.intValue == 2)
        {
            //XEditor.DrawVector3Field("heading direction:", "", SpriteXYDir);
        }
    ...
    void DrawMainConfig()
    {
        ...
        {
            string info = "Spherical Billboard's material  requires this specific shader: \n 'Xffect/Object/spherical_billboard_add'.";
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (RenderType.intValue == 0)
            ...
            DrawRope();
        else if (RenderType.intValue == 5)
            DrawSphericalBillboard();
        else if (RenderType.intValue == 6)
            DrawSplineTrail();
        else
            DrawDecal();
        ...
        if (RenderType.intValue != 6 && DirType.intValue != 4)
            DrawAffectors();
        ...
        if (needToReset && Script.gameObject.transform.parent != null)
        {
            XffectComponent parent = Script.gameObject.transform.parent.GetComponent<XffectComponent>();
            if (parent != null && parent.EditView)
            {
                parent.ResetEditScene();
            }
        }
