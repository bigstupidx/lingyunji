...
public class XEditorTool
{
    ...
    public XArea BeginCommonXArea(Editor editor, string id, GUIStyle style, float minHeight, EArea type, SerializedProperty bobj)
    {
        return BeginCommonXArea(editor, id, style, minHeight, type, bobj, 0f);
    }

    public XArea BeginCommonXArea(Editor editor, string id, GUIStyle style, float minHeight, EArea type, SerializedProperty bobj, float offset)
    {
	...
	public void DrawInfo(string info, int type = 1)
	{
    ...
    const float NOTIF_DISPLAY_TIME = 1f;
    private static float mNotifTimer = 0f;
    private static EditorWindow mNotifWindow;
    private static bool mNotifDisplayed = false;

    private static void NotifUpdate()
    {
        if (mNotifDisplayed && Time.realtimeSinceStartup > mNotifTimer)
        {
            mNotifDisplayed = false;
            RemoveNotification(mNotifWindow);
        }
    }

    public static void ShowNotification(string info)
    {
        SceneView scnview = SceneView.lastActiveSceneView;
        if (scnview == null)
            scnview = EditorWindow.GetWindow<SceneView>();

        ShowNotification(scnview, info);
    }
    ...
    public static void ShowNotification(EditorWindow window, string notif)
    {
        ...
        mNotifTimer = Time.realtimeSinceStartup + NOTIF_DISPLAY_TIME;
        mNotifWindow = window;
        mNotifDisplayed = true;
    }
    ...
    public static string GetEditorAssetPath()
    {
        ...
        {
            return EffectLayerCustom.EditorAssets;
        }
        else
        {
            return storedpath;
