    ...
    public class RibbonTrail : RenderObject
    {
        ...
        public override void Reset()
        {
            ...
            if (Node.Owner.AlwaysSyncRotation)
                headpos = Node.ClientTrans.rotation * (Node.GetRealClientPos() + Node.Owner.EmitPoint);
            else
                headpos = Node.GetRealClientPos() + Node.Owner.EmitPoint;
            SetHeadPosition(headpos);
            ResetElementsPos();
            StretchCount = 0;
            SetColor(Color.clear);
            UpdateVertices(Vector3.zero);
            mLastClientPos = Node.Owner.ClientTransform.position;
        }

        public override void Update(float deltaTime)
        {
            SetHeadPosition(Node.CurWorldPos);
            if (Node.Owner.UVAffectorEnable || Node.Owner.UVRotAffectorEnable || Node.Owner.UVScaleAffectorEnable)
                SetUVCoord(Node.LowerLeftUV, Node.UVDimensions);
            if (Node.Owner.SyncTrailWithClient)
                CheckUpdateSync();
            SetColor(Node.Color);
            MyUpdate(deltaTime);
        }
        ...
        void CheckUpdateSync()
        {
            Vector3 diff = Node.Owner.ClientTransform.position - mLastClientPos;
            mLastClientPos = Node.Owner.ClientTransform.position;

            if (Head != CHAIN_EMPTY && Head != Tail)
            {
                int laste = Head + 1; //except head.
        ...
        {
            TrailLength = len;
            ElemLength = TrailLength / (MaxElements - 2);
        ...
        void ScaleLength()
        {
            float scaleY = Node.Scale.y * Node.OriScaleY;
            //IMPORTANT: the scaleY can't be 0, or the while loop will not stop!
            scaleY = Mathf.Clamp(scaleY, 0.1f, scaleY);
            SetTrailLen(mOriLen * scaleY);
        }
        ...
        public void MyUpdate(float deltaTime)
        {
            ...
            if (Node.Owner.ScaleAffectorEnable)
                ScaleLength();
            ...
            eyePos = Node.MyCamera.transform.position;
        ...
        public void UpdateVertices(Vector3 eyePos)
        {
            ...
            //float trailLen = Node.Owner.RibbonLen;
            ...
            if (Head != CHAIN_EMPTY && Head != Tail)
            {
                ...
                for (int e = Head; ; ++e) // until break
                {
                    ...
                    if (Node.Owner.RibbonUVStretch == UV_STRETCH.Vertical)
