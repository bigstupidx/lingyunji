    ...
    public class RuntimeManager : MonoBehaviour
    {
...
#if UNITY_EDITOR
        static public Dictionary<Guid, FMOD.Studio.EventDescription> CachedDescriptions
        {
            get { return instance == null ? null : instance.cachedDescriptions; }
        }
#endif
        ...
        void Initialiase(bool forceNoNetwork)
        {
            ...
            else
            {
                ...
                //switch(fmodSettings.ImportType)
                //{
                //case ImportType.StreamingAssets:
                //    {
                ...
                //        try
                //        {
                //            LoadBankCustom(fmodSettings.MasterBank + ".strings", fmodSettings.AutomaticSampleLoading);
                //        }
                //        catch (BankLoadException e)
                //        {
                //            UnityEngine.Debug.LogException(e);
                ...
                //            try
                //            {
                //                LoadBankCustom(fmodSettings.MasterBank, fmodSettings.AutomaticSampleLoading);
                //            }
                //            catch (BankLoadException e)
                //            {
                //                UnityEngine.Debug.LogException(e);
        ...
        class AttachedInstance            
        {
            ...
            public Func<FMOD.ATTRIBUTES_3D> To3DAttributes;
            public Func<bool> isDestroy;
        }
        ...
        void Update()
        {
            if (studioSystem != null)
            {
                ...
                for (int i = 0; i < attachedInstances.Count; i++)
                {
                    ...
                    AttachedInstance ai = attachedInstances[i];
                    ai.instance.getPlaybackState(out playbackState);
                    if (!ai.instance.isValid() || playbackState == FMOD.Studio.PLAYBACK_STATE.STOPPED || (ai.isDestroy == null || ai.isDestroy()))
                    {
                    ...
                    attachedInstances[i].instance.set3DAttributes(ai.To3DAttributes());
                }
        ...
        public static void AttachInstanceToGameObject(FMOD.Studio.EventInstance instance, Transform transform, Rigidbody rigidBody)
        {
            ...
            attachedInstance.isDestroy = () => { return transform == null ? true : false; };
            attachedInstance.To3DAttributes = () => { return RuntimeUtils.To3DAttributes(transform, rigidBody); };
            attachedInstance.instance = instance;

            Instance.attachedInstances.Add(attachedInstance);
        }

        public static void AttachInstance(FMOD.Studio.EventInstance instance, Func<bool> isDestroy, Func<FMOD.ATTRIBUTES_3D> To3DAttributes)
        {
            var attachedInstance = new AttachedInstance();

            attachedInstance.isDestroy = isDestroy;
            attachedInstance.To3DAttributes = To3DAttributes;
            attachedInstance.instance = instance;
        ...
        // 完事的路径
        public static FMOD.Studio.Bank LoadBankFullPath(string fullpath, bool loadSamples = false)
        {
            string bankName = System.IO.Path.GetFileNameWithoutExtension(fullpath);
            if (Instance.loadedBanks.ContainsKey(bankName))
            {
                LoadedBank loadedBank = Instance.loadedBanks[bankName];
                loadedBank.RefCount++;
                ...
                return loadedBank.Bank;
            }
            else
            {
                LoadedBank loadedBank = new LoadedBank();
                FMOD.RESULT loadResult = Instance.studioSystem.loadBankFile(fullpath, FMOD.Studio.LOAD_BANK_FLAGS.NORMAL, out loadedBank.Bank);
                if (loadResult == FMOD.RESULT.OK)
                {
                    loadedBank.RefCount = 1;
                    Instance.loadedBanks.Add(bankName, loadedBank);
        ...
        public static void LoadBankCustom(string bankName, bool loadSamples = false)
        {
            if (Instance.loadedBanks.ContainsKey(bankName))
            {
                LoadedBank loadedBank = Instance.loadedBanks[bankName];
            ...
            else
            {
                ...
                FMOD.Studio.BANK_INFO info = new FMOD.Studio.BANK_INFO();
                BankFileLoad bfl = new BankFileLoad();
                bfl.filename = bankName;

                info.openCallback = bfl.Open;
                info.closeCallback = bfl.Close;
                info.readCallback = bfl.Read;
                info.seekCallback = bfl.Seek;
                info.userData = IntPtr.Zero;
                info.userDataLength = 0;

                FMOD.RESULT loadResult = Instance.studioSystem.loadBankCustom(info, FMOD.Studio.LOAD_BANK_FLAGS.NORMAL, out loadedBank.Bank);
                if (loadResult == FMOD.RESULT.OK)
                {
                    loadedBank.RefCount = 1;
                    Instance.loadedBanks.Add(bankName, loadedBank);
        ...
        public static FMOD.Studio.EventInstance CreateInstance(Guid guid, out FMOD.Studio.EventDescription eventDesc)
        {
            eventDesc = GetEventDescription(guid);
        ...
        public static FMOD.Studio.EventInstance CreateInstance(Guid guid)
        {
            ...
            return CreateInstance(guid, out eventDesc);
        }

        public static FMOD.Studio.EventInstance PlayOneShot(string path, Vector3 position = new Vector3())
        {
            try
            {
        ...
        public static void SetPause(bool pauseStatus)
        {
            if (instance == null)
                return;

            instance.Pause(pauseStatus);
        }

        public static FMOD.Studio.EventInstance PlayOneShot(Guid guid, Vector3 position = new Vector3())
        {
            FMOD.Studio.EventDescription desc = null;
            var instance = CreateInstance(guid, out desc);
        ...
        public static FMOD.Studio.EventInstance PlayOneShotAttached(string path, GameObject gameObject)
        {
            try
            {
                return PlayOneShotAttached(PathToGUID(path), gameObject);
            }
            catch (EventNotFoundException)
            {
                // Switch from exception with GUID to exception with path
        ...
        public static FMOD.Studio.EventInstance PlayOneShotAttached(Guid guid, GameObject gameObject)
        {
        ...
        public static FMOD.Studio.EventInstance PlayOneShotAttached(Guid guid, Func<bool> isDestory, Func<FMOD.ATTRIBUTES_3D> To3DAttributes)
        {
            var instance = CreateInstance(guid);
            AttachInstance(instance, isDestory, To3DAttributes);
            instance.start();
        ...
        {
            FMOD.Studio.EventDescription eventDesc = null;
            if (Instance.cachedDescriptions.TryGetValue(guid, out eventDesc) && eventDesc.isValid())
            {
