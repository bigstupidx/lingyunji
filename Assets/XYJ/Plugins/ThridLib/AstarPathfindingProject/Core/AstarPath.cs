...
public class AstarPath : VersionedMonoBehaviour
{
    ...
    public static System.Version Version { get { return new System.Version(4, 0, 10); } }
...
#line default
    ...
    public static int CalculateThreadCount(ThreadCount count)
    {
        ...
        if (count == ThreadCount.AutomaticLowLoad || count == ThreadCount.AutomaticHighLoad)
        {
            ...
            {
                if (memory <= 1024)
                    logicalCores = System.Math.Min(logicalCores, 2);
            }
            else
            {
    ...
    static Seeker seeker = null;

    public static void StartPath(Vector3 startPos, Vector3 endPos, System.Action<List<Vector3>> OnCall)
    {
        if (seeker == null)
        {
            seeker = (new GameObject("Seeker")).AddComponent<Seeker>();
            seeker.gameObject.AddComponent<FunnelModifier>();
        }

        Path p = seeker.StartPath(startPos, endPos);
        p.callback += (Path pt) => 
        {
            OnCall(p.vectorPath);
        };
