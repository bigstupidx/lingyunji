...
namespace Pathfinding.WindowsStore
{
	...
	public class Thread
    {
		...
		public bool IsAlive
        {
            get { return this._task != null && !this._task.IsCompleted; }
		...
		private void EnsureTask (object paramThreadStartParam = null)
        {
			if (this._task == null)
            {
                ...
                {
					this._task = new Task(delegate { this._paramThreadStart(paramThreadStartParam); }, this._taskCancellationTokenSource.Token);
				}
                else
                {
                    if (this._threadStart != null)
                    {
                        this._task = new Task(delegate { this._threadStart(); }, this._taskCancellationTokenSource.Token);
                    }
				}
			}
		}
