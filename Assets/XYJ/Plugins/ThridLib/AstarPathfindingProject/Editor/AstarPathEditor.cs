...
namespace Pathfinding
{
    ...
    public class AstarPathEditor : Editor
    {
        ...
        TextAsset SaveGraphData(byte[] bytes, TextAsset target = null)
        {
            ...
            {
                projectPath = activeScene.path.Replace('\\', '/');
                projectPath = projectPath.Substring(0, projectPath.LastIndexOf('/'));
                projectPath = string.Format("{0}/Pathfinder/{1}.bytes", projectPath, activeScene.name);

                System.IO.Directory.CreateDirectory(projectPath.Substring(0, projectPath.LastIndexOf('/')));
                System.IO.File.WriteAllBytes(projectPath, bytes);
                AssetDatabase.Refresh();
                return AssetDatabase.LoadAssetAtPath<TextAsset>(projectPath);
            }
