...
namespace NAudio.Wave
{
    ...
    public class WaveMixerStream32 : WaveStream
    {
        ...
        static /*unsafe*/ void Sum32BitAudio(byte[] destBuffer, int offset, byte[] sourceBuffer, int bytesRead)
        {
            ...
            {
                int dstpos = offset + i * 4;
                float dstV = BitConverter.ToSingle(destBuffer, dstpos);
                float srcV = BitConverter.ToSingle(sourceBuffer, i * 4);

                dstV += srcV;

                byte[] bytes = BitConverter.GetBytes(dstV);

                for (int m = 0; m < 4; ++m)
                {
                    destBuffer[dstpos + m] = bytes[m];
                }
            }

            //fixed (byte* pDestBuffer = &destBuffer[offset], pSourceBuffer = &sourceBuffer[0])
            //{
