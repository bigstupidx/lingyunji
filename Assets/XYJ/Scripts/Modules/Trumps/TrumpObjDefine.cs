...
namespace xys.hot
{
    ...
    class TrumpAttributeItem
    {
        public TrumpAttributeItem(int id, Button target, System.Action<int, AttributeDefine, Transform> clickHandle)
        {
            this.id = id;
            this.skin = target;
        ...
        public void RefreshNum(BattleAttri battleAttri)
        {
            valueText.text = AttributeDefine.GetValueStr(prop.id, battleAttri.Get(prop.id));
        }

        public void RefreshNum(BattleAttri battleAttri, BattleAttri pAddArr)
        {
            ...
            if (pAddArr.Get(prop.id) > battleAttri.Get(prop.id))
            {
                double val = System.Math.Ceiling(pAddArr.Get(prop.id)) - System.Math.Ceiling(battleAttri.Get(prop.id));
