...
namespace xys.battle
{
    ...
    class SimpleAILocalPlayer : ISimpleAI
    {
                    ...
                    if (moveInput && m_obj.battle.IsCanMove())
                    {
                        if (m_obj.battle.m_skillMgr.IsPlaying())
                        {
                            ...
                            if (aniType == Config.SkillAniConfig.AniType.Casting || aniType == SkillAniConfig.AniType.CastingContinue)
                            {
                                ...
                                {
                                    SkillManagerBase skillMgr = ((SkillManagerBase)App.my.localPlayer.battle.m_skillMgr);
                                    if (!skillMgr.m_requestStopSkill)
                                    {
                                        App.my.socket.SendGame(Protoid.A2C_Battle_StopSkill, new None());
                                        skillMgr.m_requestStopSkill = true;
                                    }
                                }
                            }
                            //技能移动
