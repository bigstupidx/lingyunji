...
namespace Config
{
    public partial class EquipInforceTab 
    {
        static List<EquipInforceTab> DataList = new List<EquipInforceTab>();
        static public List<EquipInforceTab> GetAll()
        {
            return DataList;
        }

        static public EquipInforceTab Get(int key)
        {
            ...
            CsvCommon.Log.Error("EquipInforceTab.Get({0}) not find!", key);
            return null;
        }
        ...
        public static void Load(CsvCommon.ICsvLoad reader)
        {
            if (DataList.Count != 0)
            {
                var type = typeof(EquipInforceTab);
            ...
            {
                var curType = typeof(EquipInforceTab);
                while (null != curType)
                {
            ...
            List<EquipInforceTab> allDatas = new List<EquipInforceTab>();

            {
                string file = "Item/EquipInforceTab.txt"; 
                ...
                for (int i = 3; i < reader.YCount; ++i)
                {
                    try
                    {
                        EquipInforceTab data = new EquipInforceTab();
